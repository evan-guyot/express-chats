...
class MessageController {
  static getAllMessagesByRoom: RequestHandler = async (
    ...
    try{
        ...
        res.status(404).json({ error: `We were unable to find messages for the room : ${roomId}` });
        return;
    }
  };
  ...
  static addMessageToRoom: RequestHandler = async (
    ...
    res: Response
        ...
        try {
            ...
            const userInfos = await AuthService.getTokenInfos(req.headers.authorization?.split(" ")[1]);
            ...
            const newMessage = await MessageService.addToRoom(validatedData, userInfos.id);
